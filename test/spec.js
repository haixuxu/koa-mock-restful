...
const app = new Koa();
app.use(koaBody());
app.use(mockMiddleware({entry: entry, debug: true})).
...
describe('mock api', function() {
  ...
  it('should can mock object vlaue', function(done) {
    request(server).get('/api/user').expect(200, {
      username: 'admin',
      sex: 5,
    }, done);
  });
  it('should can mock with query params', function(done) {
    request(server).get('/api/list?limit=10&offset=0').expect(200, {
      limit: 10,
      offset: 0,
      list: [
        {
  ...
  it('should can mock function', function(done) {
    request(server).get('/repos/hello').
  ...
  it('should can accept POST params', function(done) {
    request(server).post('/api/login/account').
  ...
  it('should can accept DELETE params', function(done) {
    request(server).delete('/api/user/122').
